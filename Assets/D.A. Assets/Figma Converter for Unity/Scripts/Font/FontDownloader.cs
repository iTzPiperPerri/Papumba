...
namespace DA_Assets.FCU
{
    ...
    public class FontDownloader : MonoBehaviourBinder<FigmaConverterUnity>
    {
        ...
        public void PrintFontNames(FcuLocKey locKey, List<FontStruct> fonts)
        {
            List<string> fontNames = new List<string>();

            foreach (FontStruct item in fonts)
            {
                if (item.FontSubset == FontSubset.Latin)
                {
                    fontNames.Add(item.FontMetadata.FontNameToString(true, true, null, false));
                }
                else
                {
                    fontNames.Add(item.FontMetadata.FontNameToString(true, true, item.FontSubset, false));
                }
            }

            DALogger.Log(locKey.Localize(fontNames.Count, $"\n{string.Join("\n", fontNames)}"));
        }


        [SerializeField] TmpDownloader tmpDownloader;
        [DASerialization(nameof(tmpDownloader))]
        public TmpDownloader TmpDownloader => tmpDownloader.SetMonoBehaviour(monoBeh);

        [SerializeField] TtfDownloader ttfDownloader;
        [DASerialization(nameof(ttfDownloader))]
        public TtfDownloader TtfDownloader => ttfDownloader.SetMonoBehaviour(monoBeh);

        [SerializeField] DaGoogleFontsApi gFontsApi;
        [DASerialization(nameof(gFontsApi))]
