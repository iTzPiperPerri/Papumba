...
namespace DA_Assets.FCU.Extensions
{
    public static class FcuExtensions
    {
        ...
        public static IEnumerator WriteLog(this Request request, string text, string add = null)
        {
            string logPath = FcuConfig.Instance.LogPath;
            ...
            JFResult jfr = DAFormatter.Format(text);

            string formated = jfr.Json;
            formated = $"{request.Query}\n{formated}";

            File.WriteAllText(logFilePath, formated);

            yield return null;
        }

        public static bool IsProjectEmpty(this FigmaProject figmaProject)
        {
            try
            {
                if (figmaProject.Equals(default) ||
                    figmaProject.Document.Equals(default(FObject)) ||
        ...
        public static Color SetFigmaAlpha(this Color color, float? opacity) =>
            new Color(color.r, color.g, color.b, opacity == null ? 1 : opacity.ToFloat());
