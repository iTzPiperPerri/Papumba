...
namespace DA_Assets.FCU
{
    ...
    public class HashGenerator : MonoBehaviourBinder<FigmaConverterUnity>
    {
        ...
        private void GetHashRecursive(FObject fobject, List<FObjectHashData> values, int indent)
        {
          ...
          /*  try
            {*/
                ...
                monoBeh.Log($"IsInsideDownloadable | {fobject.Data.Hierarchy} | {downloadableFObject.Name}");
        ...
        private static string ConvertToTreeString(List<FObjectHashData> objectHashes, bool includeNames)
        {


            StringBuilder sb = new StringBuilder();

            foreach (var objectHash in objectHashes)
            {
                if (includeNames)
                {
