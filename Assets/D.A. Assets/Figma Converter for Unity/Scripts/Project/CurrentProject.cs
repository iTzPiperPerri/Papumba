...
namespace DA_Assets.FCU
{
    ...
    public class CurrentProject : MonoBehaviourBinder<FigmaConverterUnity>
    {
        ...
        public FObject GetByIndex(int index)
        {
            if (index == -1)
            {
                return default;
        ...
        public FObject GetParent(FObject fobject)
        {
            if (fobject.Data.ParentIndex == -1)
            {
                return default;
            }

            return this.CurrentPage[fobject.Data.ParentIndex];
        }
        ...
        public FObject GetRootFrame(FObject fobject)
        {
            ...
            FObject parent = GetParent(fobject);
        ...
        public FObject GetNotDuplicate(FObject fobject)
        {
            ...
            return GetRootFrame(GetParent(fobject));
        }
        ...
        {
            Parallel.For(0, fobjects.Count, i =>
            {
                if (fobjects[i].ContainsTag(FcuTag.Page))
                    return;

                fobjects[i].Data.RootFrame = monoBeh.CurrentProject.GetRootFrame(fobjects[i]).Data;
            });
        }
