...
namespace DA_Assets.FCU
{
    ...
    public class Authorizer : MonoBehaviourBinder<FigmaConverterUnity>
    {
        ...
        {
            RoutineResult<AuthResult, FigmaError> authResult = default;

            yield return StartAuthThread(x => authResult = x);

            if (authResult.Success)
            {
                monoBeh.FigmaSession.AddNew(authResult.Result.access_token);
            }
            else
            {
                DALogger.LogError(FcuLocKey.log_cant_auth.Localize(authResult.Error.Error, authResult.Error.Status));
            }
        }

        private IEnumerator StartAuthThread(Return<AuthResult, FigmaError> @return)
        {
            string code = "";
