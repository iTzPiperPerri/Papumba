...
namespace DA_Assets.FCU
{
    ...
    public class TransformSetter : MonoBehaviourBinder<FigmaConverterUnity>
    {
        ...
        public Rect GetRect(FObject fobject)
        {
            ...
            {
                FObject parent = monoBeh.CurrentProject.GetByIndex(fobject.Data.ParentIndex);

                if ((parent.IsDefault() == false && 
                    parent.ContainsTag(FcuTag.AutoLayoutGroup)) || 
                    rSize.IsBiggerThan(bSize))
                {
                    state = 3;
